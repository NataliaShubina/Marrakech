...
int CCharacterFactory::LoadObjects(const char * filepath)
{
	...
	const Json::Value& jchar = jobj["Characters"];
	m_uObjectNum = jchar.size();

	m_pSceneObjects = new CObject *[m_uObjectNum];

	const Json::Value& jcharPreset = jobj["CharacterPresets"];
	...
	for (size_t i = 0; i < m_uObjectNum; i++)
	{
		...
		float y = static_cast<float> (rand()) / static_cast<float>(RAND_MAX / 3.0f);
		float z = static_cast<float> (rand()) / static_cast<float>(RAND_MAX / 3.0f);
		...
		float size = static_cast<float> (rand()) / static_cast<float>(RAND_MAX / 0.01f);
